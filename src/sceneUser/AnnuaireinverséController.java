 ...
 * @author inclu
 */
public class AnnuaireinverséController implements Initializable {
           ...
           String textRecherche = txtRechercher.getText().replace(","," ");
            ...
            for (Etudiant e: loadTxt.getData()){
                ...
                for(String a:tabTexteRecherche){
                    ...
                    if (e.getContact().toLowerCase().contains(a)){
            ...
            for (Map.Entry<Integer, Integer> entry: dictIDandOccurence.entrySet()){
                for(Etudiant e: tempRes){
                    if(entry.getValue()>=tabTexteRecherche.length){
        ...
        txtRechercher.textProperty().addListener(new ChangeListener<String>() {
            public void changed(ObservableValue<? extends String> observable, String oldValue,
                ...
                if (!newValue.matches("\\d*")) {
                    txtRechercher.setText(newValue.replaceAll("[^\\d]", " "));
                }
            }
        });
